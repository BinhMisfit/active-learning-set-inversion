...
# -*- coding: utf-8 -*-
"""
Created on Mon Feb 25 10:50:31 2019
Same as Set_Inversion_Lotka
Used to solve Lotka Voltera problem by using SVM, KNN, MLP, RF
@author: duynguyen
"""
...
X_samp, Y_samp = RL.Random_Lotka(K_samp, Link_GT)    
...
for idx_method in method_numb_idx:
    ...
    for g in range(G_Points.shape[0] + 1):
        ...
        if (g < G_Points.shape[0]):
            ...
            while break_flag:
                # ---------- RANDOM POINT ----------
                initial_guess = np.random.uniform(0.01001, 0.08,int(Shape[0]))                          
            ...
            # find true lable by finding nearest groundtruth point
            Dis = np.linalg.norm(C_Mesh - g_points, axis = 1)
            minDis = np.argmin(Dis)
            tl = CT[minDis]
    ...
    filename = 'Model_' + method[idx_method] + '_LV_' + Random_Type + '_' + Random_Type_Generate + '_' + str(K_samp + K_bound) + '_' + str(Active_Points) + '.sav'
    ...
    filename_fig = 'Points_Figure_' + method[idx_method] + '_LV_' + Random_Type + '_' + Random_Type_Generate + '_' + str(K_samp + K_bound) + '_' + str(Active_Points) + '.pkl'
    ...
    filename_csv = 'Result_Accuracy_Optimal_LV_' + Random_Type + '_' + Random_Type_Generate + '_' + str(K_samp + K_bound) + '_' + str(Active_Points) + '.csv'
...
filename_csv = 'Result_Optimal_Full_LV_' + Random_Type + '_' + Random_Type_Generate + '_' + str(K_samp) + '_' + str(Active_Points) + '.csv'
optimal_pd.to_csv(Link_CSV + filename_csv, sep='\t', encoding='utf-8')
filename_csv = 'Result_Optimal_LV_' + Random_Type + '_' + Random_Type_Generate + '_' + str(K_samp) + '_' + str(Active_Points) + '.csv'
